...
int main(int argc, const char *argv[]) 
{
    ...
    // build_graph restituisce circuit_graph<int>
    ...
    auto cycles_raw = de_pina(cg.get_graph());

    // plus_minus richiede che il nodo iniziale sia ripetuto in coda: [n0,...,nk,n0]
    std::vector<std::vector<int>> cycles;
    for (auto c : cycles_raw) {
        c.push_back(c.front());
        cycles.push_back(c);
    }
